**Update:** We have added two items to the 'idiomatic names' which are relevant to our domain. ES = sequence of events and SS = sequence of signals.
 ...
 ** `x`, `y` - numbers
 ** `xs` - sequence
 ** `es` - sequence of events
 ** `ss` - sequence of signals
 ** `m` - map
 ** `k`, `ks` - key, keys
